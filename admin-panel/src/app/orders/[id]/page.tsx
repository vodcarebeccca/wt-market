...
export default async function AdminOrderDetailPage({ params, searchParams }: Props) {
  ...
  // Link to the storefront (separate app) where the buyer views their order.
  const deliveryUrl = `${process.env.STOREFRONT_URL || "https://wt-market.vercel.app"}/${order.locale}/order/${order.code}?token=${order.accessToken}`;
